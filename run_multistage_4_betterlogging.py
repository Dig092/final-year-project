...
def create_gpt_agent(name, system_message, llm_config):
    ...
    text_compressor = TextMessageCompressor(
        ...
        compression_params={"target_token": 64000},
        cache=None,
    )
...
class InitialPlanner():
    ...
    def create_required_agents(self):
        ...
        self.planner = create_gpt_agent("Planner", system_message=plannerphase_planner_system_message, llm_config=gpt4_config)
    ...
    def setup_groupchat(self):        
        ...
        self.manager = autogen.GroupChatManager(groupchat=self.groupchat, llm_config=gemini_config)
...
- Optimize the code for multiprocessing.
...
class DataEngineer():
    ...
    def create_required_agents(self):
        ...
        self.data_engineer = create_gpt_agent("DataEngineer", system_message = data_engineer_system_message, llm_config = gpt4_config)
        ...
        self.summarizer = create_gpt_agent("Summarizer", system_message=data_summarizer_system_message, llm_config = gpt4_config)
    ...
    def get_planner_summary(self):
        ...
        upgraded_prompt = f"""
            ...
            Suggestions for ML Team:
            - Run small experiments first to validate the success of the approach and then finalize the hyperparameters to be tuned. Only then run complete experiments to achieve best results.
            """
        return upgraded_prompt


llm_preference = """
When required to use transformer architecture models then if a model is not specified by admin then you can explore to use latest small language models such as Gemma 2 2B or Llama 3.2 1B as they perform really well on such tasks out of the box and can be further improved. Use retreive_from_internet tool if needed to gather context about these models and code to use them.
"""

training_code_guideline = f"""
Code Generation Guidelines for Training or Fine-Tuning a Model:
* Always provide the full code for direct copy-paste; do not omit any parts or ask to keep existing functions.
* Initialize model and optimizer once before training loop; maintain their state throughout epochs using a single consistent checkpoint path.
* Include code for regular checkpointing of weights and save the final weights after execution.
* Store model checkpoints and weights in the `/tmp/model` directory; create it if it doesn't exist.
* Add logging and formatting for each training step, iteration, epoch, or batch and loss metrics. Save the logs and experiment outcomes to scratchpad using `add_to_scratchpad` tool for further assessment.
* Select an appropriate model size based on the data; aim for the most optimal size.
* Suggest hyperparameters for faster convergence through small experiments.
* Implement early stopping to prevent overfitting.
* Begin with small experiments to validate the approach and finalize hyperparameters, then run complete experiments for best results.
* Optimize the data loading, processing and training code for multiprocessing.
* Never reload base model during training; only load best checkpoint after completion for evaluation.
...
Model preferences:
For boosting performance on tasks like text classification, identification, question-answering, and chat completion, prefer to use transformers or faster altenatives like SLMs over LSTMs.
"""

# explore to use ensemble models combining large language models like LLaMA 3.2 1B, Gemma 2 2B, Mistral 7B v3, and transformer architectures such as BERT, RoBERTa, or GPT models.

lead_machine_learning_engineer_system_message = f"""
You are a Lead ML Engineer and our task is to plan the training of models to beat the problem statement.
...
Use `retrieve_from_scratchpad` tool to assess the current progress and check if we have achieved the target. If not then iterate and improve plan for team.
Provide a clear unambigious step by step instruction to junior machine learning engineer to write the code.

Experiment guidelines:
In the beginning always suggest to run very small experiments as a starting point for the team.
Baed on the results of the smaller experiments, suggest the next steps to decide on models or hyperparameters.
Make sure to optimize the batchsize for best/faster training and then proceed to complete model finetuning.
Always suggest the team to use Pytorch, Multiprocessing and GPU for faster execution.
Iteratively keep exploring and improving the approach unless Target is achieved or unless Admin suggests to stop the process or do something else.

Model training suggestion guidelines:
The model checkpoints or weights must be stored in `/tmp/model` directory.
Suggest to use latest optimized architecture models to solve complex problem statements such as transformer models, emsemble models, latest Large Language models and NLP models if needed to maximize improvements.
"""
...
class MachineLearningEngineer():
    ...
    def create_required_agents(self):
        ...
        self.lead_machine_learning_engineer = create_gpt_agent("LeadMachineLearningEngineer", system_message=lead_machine_learning_engineer_system_message, llm_config=gpt4_config)
    ...
    def register_function_calls(self):
        autogen.register_function(retreive_from_internet, caller=self.lead_machine_learning_engineer, executor=self.executor, name="retreive_from_internet", description="Search internet and find context from internet.")
        ...
        autogen.register_function(add_to_scratchpad, caller=self.junior_machine_learning_engineer, executor=self.executor, name="add_to_scratchpad", description="Add any training logs or eval metrics for managing progress across experiments.")
        autogen.register_function(add_to_scratchpad, caller=self.hyperparam_tuner, executor=self.executor, name="add_to_scratchpad", description="Add any training logs or eval metrics for managing progress across experiments.")
        autogen.register_function(add_to_scratchpad, caller=self.debugger, executor=self.executor, name="add_to_scratchpad", description="Add any training logs or eval metrics for managing progress across experiments.")
    ...
    def setup_groupchat(self):
        ...
        teachability.add_to_agent(self.lead_machine_learning_engineer)
        teachability.add_to_agent(self.hyperparam_tuner)

        self.groupchat = autogen.GroupChat(
        agents=[self.lead_machine_learning_engineer, self.junior_machine_learning_engineer, self.executor,self.debugger,self.hyperparam_tuner],
        messages=[],
        max_round=200,
        select_speaker_message_template = """You are in a role play game. The following roles are available:
                    {roles}.
        ...
        select_speaker_prompt_template = "Read the above conversation. Select the next best role from {agentlist} to achieve the solution. Only return the role.",
        )
        self.manager = autogen.GroupChatManager(groupchat=self.groupchat, llm_config=gemini_config)
