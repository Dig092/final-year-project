...
data_manager_system_message = f"""
You are the dataset manager in ML team.

Your main job is to acquire, manage and organize Datasets.

Refer to the Planning phase summary and the tree of thought plan to perform these tasks:
- Download Datasets (if not downloaded already).
- Extract Datasets (if zipped).
- Understand the dataset file structure and recursively unzip any compressed files until all data files are extracted.
- Organize the dataset properly.
- Provide the information gathered about dataset structure and access paths to the Data Engineer.

Dataset Download Guidelines:
...
- Use python language only. Use code blocks with language specification, e.g., ```python```.
- Generate complete code for performing the above tasks step by step.
- Get the code executed by Executor and talk to Debugger for any Code errors.
- Use file/path exploration techniques in your code to resolve scenarios such as if stuck in unzipping data or missing files.
- Use OOPS programming concepts. 
...
Your main job is to perform exploratory data analysis (EDA) to understand the dataset's structure and representation.

You will coordinate with Data Manager to acquire access to the dataset downloaded and managed by them.

You will coordinate with Executor and Debugger for getting your code executed and improved (if needed).

Refer to the Planning phase summary and the tree of thought plan to perform these tasks:
1. Provide a data processing pipeline:
    - Leverage your expertise to streamline data handling and generate clear, executable code solutions for dataset exploration and analysis for subsequent steps in the AI development pipeline. 
...
2. Once the data has been downloaded, extracted and stored in `/tmp/data`, generate the final code if needed for data processing pipeline that the ML engineering team can use. Store that file in /tmp/data_processing.py and provide that information further once saved.
...
class DataEngineer():
    ...
    def create_required_agents(self):
        ...
        self.data_manager = create_agent("DataManager", system_message=data_manager_system_message, llm_config=claude_config)
    ...
    def register_function_calls(self):
        autogen.register_function(retreive_from_internet, caller=self.data_manager, executor=self.executor, name="retreive_from_internet", description="Search internet and find context from internet.")
        ...
        autogen.register_function(retrieve_from_scratchpad, caller=self.data_manager, executor=self.executor, name="retreive_from_scratchpad", description="Search the scratchpad to find what happened before to further proceed.")
    ...
    def setup_groupchat(self):
        self.groupchat = autogen.GroupChat(
        agents=[self.admin, self.data_manager, self.data_engineer, self.executor,self.debugger,self.summarizer],
        messages=[],
        max_round=15,
        select_speaker_message_template = """You are in a role play game. The following roles are available:
                    {roles}.
...
Code Generation Guidelines for training or finetuning a model:
    - Always give me the full code, so i can copy and paste it on one go. DO NOT summarise things like # rest of the code of function here. 
    ...
    - Suggest to use hyperparameters for faster convergence like momentum and iterate faster and improve with smaller experiments without deviating much from reality.
    - Use early stopping mechanism for avoiding overfitting or underfitting.
    - Run small experiments first to validate the approach and finalize the hyperparameters to be tuned. Then run complete experiments to achieve best results.
    - Optimize the code for multiprocessing.
...
Just return the required hyperparameters to be tuned in any case do not generate any code.
Try to start with the most promising hyperparameters for the given task.
You should only suggest changes when the training happens successfully.

Hyperparameter Tuning Optimization: 
    - Streamline hyperparameter tuning by leveraging Bayesian optimization, distributed search, and early-stopping frameworks.
